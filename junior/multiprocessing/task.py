...
def clean_text(text: str, lemmatizer: WordNetLemmatizer) -> str:
    """
    Clean text
...
def clear_data(source_path: str, target_path: str, n_jobs: int) -> None:
    ...
    with Parallel(n_jobs=n_jobs, backend='multiprocessing', prefer='processes') as parallel:
        cleaned_text_list = parallel(
            [delayed(clean_text)(text, lemmatizer) for text in data['text']]
        )
